...
| 2026-09-05 | sft (attempt 6) | `TypeError: list indices must be integers or slices, not tuple` in `unsloth_zoo/loss_utils.py _unsloth_get_batch_samples` — Unsloth's pre-step runs `labels[..., 1:] != -100` (ellipsis slice) which needs a **torch tensor**; v0.7.3's collator padded correctly but returned plain Python lists. | v0.7.4: `pad_collator` wraps every field in `torch.tensor(..., dtype=torch.int64)` (list fallback if torch is unimportable). **SFT stage then PASSED end-to-end** (train + eval + adapter + merge + export). |
| 2026-09-05 | dpo (attempt 1) | `ValueError: The batch received was empty, your model won't be able to train on it` at `transformers/trainer.py _prepare_inputs` — **not a data bug**: Unsloth's patched `Trainer.get_batch_samples` (`unsplash_zoo/loss_utils.py _unsloth_get_batch_samples`) tracks item-counts only for its own key set (`input_ids`, `attention_mask`, `labels`, `position_ids`) and reassembles the batch around those; our DPO dicts use `c_input_ids`/`c_attn`/`c_labels`/`j_input_ids`/`j_attn`/`j_labels`, none of which it recognizes, so `len(inputs)==0` reached `_prepare_inputs`. SFT worked because its keys are exactly the stock ones. | v0.7.5 (this commit): DPO trainer subclass overrides `get_batch_samples` to match the stock transformers 5.5.0 contract — returns `(list_of_batches, None)`. Verified against the stock source in the box's transformers (signature `-> tuple[list, torch.Tensor \| int \| None]`; caller does `for i, inputs in enumerate(batch_samples)`), so the collated 6-key dicts flow straight to `compute_loss`. Returning `None` as item-count keeps standard loss scaling. |
